...
angular
    ...
    function (
      ...
      var show_wait_for_sip_alert = function () {
        Alert.alerts.push({
          ...
          message: gettextCatalog.getString(
            "Vui lòng đợi trong khi SIP được khởi động"
          ),
          show_spinner: true,
        });
      };
      ...
      vm.create_sip_from_tag = function () {
          ...
          if (
            !confirm(
              gettextCatalog.getString(
                "Bạn có chắc chắn muốn hủy thư mục hiện tại không?"
              )
            )
          ) {
            return;
        ...
        SipArrange.create_directory(
          ...
          let unique_directories = Object.keys(
            files_grouped_by_directory
          ).filter(function (directory, i, arr) {
            let is_repeated = false;
            arr.forEach(function (other_directory) {
              if (
                other_directory !== directory &&
                other_directory.startsWith(directory)
              ) {
                is_repeated = true;
              }
            });
          ...
          Object.keys(files_grouped_by_directory).forEach((directory) => {
            let directory_arrange_path =
              "/arrange/" + full_path + "/" + directory + "/";
            files_grouped_by_directory[directory].forEach((file) => {
              let paths = generate_files_list(
                file,
                "/originals/",
                directory_arrange_path
              );
              arrange_sources = arrange_sources.concat(paths.source);
              arrange_destinations = arrange_destinations.concat(
                paths.destination
              );
            });
          });
          ...
          let on_failure = (error) => {
            Alert.alerts.push({
              type: "danger",
              message: gettextCatalog.getString(
                "Không thể khởi động SIP! Kiểm tra nhật ký bảng điều khiển."
              ),
            });
          };
          ...
          Promise.all(create_directory_promises).then(function () {
            Promise.all(copy_to_arrange_promises).then(function () {
              SipArrange.start_sip("/arrange/" + full_path + "/")
      ...
      vm.create_directory = (parent) => {
        ...
        SipArrange.create_directory("/arrange/" + full_path, path, parent).then(
          (result) => {
            target.push(result);
          }
        );
      ...
      vm.start_sip = (directory) => {
        ...
        var on_failure = (error) => {
          ...
          if (
            ...
            error.data.hasOwnProperty("code") &&
            "ERR_NO_FILES" == error.data.code
          ) {
        ...
        show_wait_for_sip_alert();
        SipArrange.start_sip("/arrange/" + directory.path + "/").then(
          on_success,
          on_failure
        );
      };
            ...
            $.ajax({
              ...
              error: (error) => {
                Alert.alerts.push({
                  type: "danger",
                  message: gettextCatalog.getString(
                    'Không thể gửi chỉnh sửa để ghi lại "{{title}}"; kiểm tra nhật ký bảng điều khiển.',
                    { title: node_title }
                  ),
                });
              },
      ...
      var on_copy_failure = (error) => {
        Alert.alerts.push({
          type: "danger",
          message: gettextCatalog.getString(
            "Không thể sao chép tập tin vào sắp xếp SIP; kiểm tra nhật ký Bảng điều khiển."
          ),
        });
      };
  ...
  .controller("ArrangementEditMetadataController", [
    ...
    "levels",
    "level",
    function ($uibModalInstance, levels, level) {
      var vm = this;
