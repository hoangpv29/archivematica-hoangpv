...
angular
  .module("archivesSpaceController", [
    "alertService",
    "sipArrangeService",
    "transferService",
    "ui.bootstrap",
    ...
    function (
      ...
      $scope.get_rights_url = (record) => {
        ...
        if (
          ...
          record.type !== "resource_component"
        ) {
          return "";
        }
      ...
      var levels_of_description =
        ArchivesSpace.get_levels_of_description().$object;
      ...
      $scope.edit = (node) => {
        ...
        form.result.then((result) => {
          ...
          var on_failure = (error) => {
            ...
            Alert.alerts.push({
              ...
              message: gettextCatalog.getString(
                'Không thể gửi sửa đổi để ghi lại "{{title}}"; Kiểm tra nhật ký bảng điều khiển.',
                { title: title }
              ),
            });
          };
          ...
          ArchivesSpace.edit(node.id, node_to_post).then(
            on_success,
            on_failure
          );
        });
        ...
        form.result.then((result) => {
          ...
          var on_failure = (error) => {
            Alert.alerts.push({
              type: "danger",
              message: gettextCatalog.getString(
                "Không thể thêm bản ghi con mới vào bản ghi {{id}}",
                { id: node.id }
              ),
            });
          };
      ...
      $scope.add_digital_object = (node) => {
        ...
        var on_failure = (result) => {
          Alert.alerts.push({
            type: "danger",
            message: gettextCatalog.getString(
              "Không thể thêm thành phần đối tượng kỹ thuật số mới để ghi {{id}}",
              { id: node.id }
            ),
          });
        };

        ArchivesSpace.create_digital_object_component(node.id, result).then(
          on_success,
          on_failure
        );
      };
      ...
      var load_element_children = (node) => {
        ...
        var on_failure_aspace = (error) => {
          Alert.alerts.push({
            type: "danger",
            message: gettextCatalog.getString(
              "Không thể tìm nạp bản ghi {{id}} từ ArchivesSpace!",
              { id: node.id }
            ),
          });
          $scope.loading = false;
        ...
        var on_failure_arrange = (error) => {
          Alert.alerts.push({
            type: "danger",
            message: gettextCatalog.getString(
              "Không thể tìm nạp bản ghi {{path}} từ Arrangement!",
              { path: node.path }
            ),
          });
          $scope.loading = false;
        ...
        if (node.id && node.type !== "digital_object") {
          // ArchivesSpace node
          ...
          ArchivesSpace.digital_object_components(node.id).then(
            (components) => {
      ...
      $scope.on_toggle = function (node, expanded) {
        if (
          (!expanded || !node.has_children || node.children_fetched) &&
      ...
      $scope.load_data = (query) => {
        ...
        let on_failure = (response) => {
          ...
          Alert.alerts.push({
            type: "danger",
            message: gettextCatalog.getString(
              "Không thể truy cập ArchivesSpace; Kiểm tra nhật ký bảng điều khiển!"
            ),
          });
        };
      ...
      var generate_files_list = (file, source_path, destination_path) => {
        ...
        if (file.children) {
          angular.forEach(file.children, (child) => {
            let child_paths = generate_files_list(
              child,
              source_path,
              destination_path + basename(file.relative_path) + "/"
            );
            paths.source = paths.source.concat(child_paths.source);
            paths.destination = paths.destination.concat(
              child_paths.destination
            );
          });
        }
        return paths;
      ...
      var copy_backlog_to_aspace = function (file) {
        ...
        let paths = generate_files_list(file, "/originals/", this.path);
        ...
        $scope.$apply(() => {
          ...
          SipArrange.copy_to_arrange(paths.source, paths.destination).then(
            on_copy
          );
        });
      };
      ...
      $scope.finalize_arrangement = function (node) {
        var on_success = () => {
          ...
          Alert.alerts.push({
            type: "success",
            message: gettextCatalog.getString(
              'Khởi động thành công SIP từ bản ghi "{{title}}"',
          ...
          node.children = node.children.filter(
            (element) => element.type !== "digital_object"
          );
        };
        var on_failure = (error) => {
          node.request_pending = false;
          ...
          if (
            error.data.message &&
            error.data.message.startsWith("Không có SIP Sắp xếp")
          ) {
            message = gettextCatalog.getString(
              'Không thể khởi động SIP; không có tập tin nào được sắp xếp vào hồ sơ "{{title}}".',
              { title: node.display_title }
            );
          } else {
            message = gettextCatalog.getString(
              "Không thể khởi động SIP; kiểm tra bảng điều khiển."
            );
          }
          Alert.alerts.push({
            type: "danger",
